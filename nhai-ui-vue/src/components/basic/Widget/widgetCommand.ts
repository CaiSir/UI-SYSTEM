...
export class NhaiWidgetCommand extends BaseCommand<WidgetOptions, WidgetEvents> {
  ...
  protected override update(): void {
    ...
    // 在更新后，确保子组件仍然存在（Vue 重新渲染时可能丢失）
    if (this._mounted && this._children.length > 0) {
      nextTick(() => {
        // 重新查找 contentContainer（可能在 Vue 重新渲染后位置变化）
        if (this._element) {
          const newContentContainer = this._element.querySelector('.widget-content') as HTMLElement
          if (newContentContainer && newContentContainer !== this.contentContainer) {
            // contentContainer 位置变化了，需要恢复子组件
            const oldContentContainer = this.contentContainer
            this.contentContainer = newContentContainer
            
            // 如果旧的 contentContainer 中有子组件，移动到新的
            if (oldContentContainer) {
              const children = Array.from(oldContentContainer.childNodes)
              children.forEach(child => {
                if (child instanceof HTMLElement) {
                  ...
                  if (child.classList && 
                    ...
                    // 这是子组件本身，需要检查是否在 childElements 映射中
                    let foundChild = false
                    for (const [childInstance, element] of this.childElements.entries()) {
                      if (element === child || element.contains(child)) {
                        foundChild = true
                        // 更新映射到新容器中的元素
                        if (newContentContainer.contains(child)) {
                          this.childElements.set(childInstance, child)
                        } else {
                          newContentContainer.appendChild(child)
                        }
                        break
            ...
            this.contentContainer = newContentContainer || this._element
          }
          
          // 确保所有子组件都已渲染（非可视化编辑器模式）
          const hasManualChildren = this.contentContainer && Array.from(this.contentContainer.children).some(
          ...
          if (!hasManualChildren) {
            ...
            this._children.forEach(child => {
              ...
              if (!existingElement || !this.contentContainer?.contains(existingElement)) {
                // 子组件丢失或不在容器中，重新渲染
  ...
  override addChild<C extends BaseCommand<any, any>>(child: C): this {
    ...
    // 如果组件已挂载，尝试渲染子组件
    if (this._mounted) {
      // 如果 contentContainer 还没有找到，尝试查找
      if (!this.contentContainer && this._element) {
        this.contentContainer = this._element.querySelector('.widget-content') as HTMLElement || this._element
      }
      
      // 如果找到了 contentContainer，尝试渲染
      if (this.contentContainer) {
        ...
        // 如果 contentContainer 还没找到（可能在 nextTick 中），延迟渲染
        nextTick(() => {
          if (!this.contentContainer && this._element) {
            this.contentContainer = this._element.querySelector('.widget-content') as HTMLElement || this._element
          }
          if (this.contentContainer && !this.childElements.has(child)) {
            const hasManualChildren = Array.from(this.contentContainer.children).some(
              c => c.classList && 
              (c.classList.contains('widget-child-component') || 
               c.classList.contains('dialog-child-component'))
  ...
  private renderAllChildren(): void {
    ...
    // 如果有手动添加的子组件（可视化编辑器模式），跳过自动渲染
    if (hasManuallyAddedChildren) {
      return
    }
    
    // 渲染所有子组件
    this._children.forEach(child => {
      // 如果子组件已经在映射中且是 wrapper，跳过
      const existingElement = this.childElements.get(child)
      if (existingElement && existingElement.classList && 
          (existingElement.classList.contains('widget-child-component') || 
           existingElement.classList.contains('dialog-child-component'))) {
        return
      }
  ...
  protected doRender(): HTMLElement {
    ...
    const WidgetWrapper = defineComponent({
      setup() {
        return () => h(Widget, {
          // 不使用 key，避免重新创建组件导致子组件丢失
          // 使用 key 只在 fullscreen 变化时（这会改变组件结构）
          key: fullscreenRef.value ? 'fullscreen' : undefined,
    ...
    // 在 nextTick 中也尝试查找（确保 Vue 组件已完全渲染）
    nextTick(() => {
      if (!this.contentContainer || this.contentContainer === container) {
        this.contentContainer = container.querySelector('.widget-content') as HTMLElement || container
      }
      
      // 如果不是可视化编辑器模式（没有手动添加的 wrapper 子组件），自动渲染所有子组件
      const hasManualChildren = this.contentContainer && Array.from(this.contentContainer.children).some(
        child => child.classList && 
        (child.classList.contains('widget-child-component') || 
         child.classList.contains('dialog-child-component'))
      ...
      // 只有在非可视化编辑器模式下（没有手动添加的子组件）才自动渲染
      if (!hasManualChildren && this._children.length > 0) {
