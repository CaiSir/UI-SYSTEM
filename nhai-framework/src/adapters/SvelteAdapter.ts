...
export class SvelteAdapter implements NHAIFrameworkAdapter {
  name = 'svelte'

  createElement(tag: string, props?: any, children?: any[]): HTMLElement {
    // 在Svelte环境中，我们仍然创建真实的DOM元素
...
export function createNHAIStore() {
  ...
  let adapter = new SvelteAdapter()

  return {
    subscribe: (callback: (value: any) => void) => {
      callback({ framework, adapter })
      return () => {}
    },
    setFramework: (newFramework: string) => {
...
export function nhaiAction(node: HTMLElement, _options: any) {
  const adapter = new SvelteAdapter()
  
  return {
    update: (newOptions: any) => {
      if (newOptions.element) {
        adapter.mount(newOptions.element, node)
      }
    },
    destroy: () => {
      // 清理工作
